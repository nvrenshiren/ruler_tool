...
//  * @private
//  */
export default function compressDracoMeshes(gltf, options) {
...
function compress(gltf, options, encoderModule, decoderModule) {
  ...
  const compressionLevel = dracoOptions.compressionLevel ?? defaults.compressionLevel
  const uncompressedFallback = dracoOptions.uncompressedFallback ?? defaults.uncompressedFallback
...
function getQuantizationBits(dracoOptions) {
  const defaults = compressDracoMeshes.defaults
  return {
    POSITION: dracoOptions.quantizePositionBits ?? defaults.quantizePositionBits,
    NORMAL: dracoOptions.quantizeNormalBits ?? defaults.quantizeNormalBits,
    TEXCOORD: dracoOptions.quantizeTexcoordBits ?? defaults.quantizeTexcoordBits,
