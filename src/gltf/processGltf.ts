...
//  */
export default function processGltf(gltf, options) {
  ...
  options.separateTextures = (options.separateTextures ?? defaults.separateTextures) || options.separate
  ...
  const preStages = [addPipelineExtras, readResources, updateVersion, addDefaults]
