import { defined as _defined, DeveloperError as _DeveloperError } from "cesium"
import { outputFile as _outputFile } from "fs-extra"
...
const DeveloperError = _DeveloperError
...
function obj2gltf(objPath, options) {
  ...
  options.doubleSidedMaterial = defaultValue(options.doubleSidedMaterial, defaults.doubleSidedMaterial)
...
function getDefaultWriter(outputDirectory) {
  if (defined(outputDirectory)) {
    return function (file, data) {
      ...
      return _outputFile(outputFile, data)
    }
  }
}
