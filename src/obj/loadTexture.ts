...
import { decode as _decode } from "jpeg-js"
...
function loadTexture(texturePath, options) {
  ...
  return readFile(texturePath).then(function (source) {
    const name = basename(texturePath, extname(texturePath))
...
function decodePng(texture, options) {
  ...
  if (decode) {
    return parsePng(source).then(function (decodedResults: any) {
...
function decodeJpeg(texture, options) {
  if (options.decode) {
    const source = texture.source
    const decodedResults = _decode(source)
    texture.pixels = decodedResults.data
    texture.width = decodedResults.width
    texture.height = decodedResults.height
    if (!options.keepSource) {
