import { Math as _Math, clone as _clone, combine as _combine, defined as _defined } from "cesium"
import { dirname, normalize, resolve, join, basename } from "path"
...
function loadMtl(mtlPath, options) {
  ...
  const mtlDirectory = dirname(mtlPath)
  const materials: any[] = []
  ...
  function normalizeTexturePath(texturePath, mtlDirectory) {
    ...
    return normalize(resolve(mtlDirectory, texturePath))
  }
  ...
  function parseLine(line) {
    ...
    if (/^newmtl/i.test(line)) {
      ...
      material.ambientColor = [parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]), 1.0]
    } else if (/^Ke /i.test(line)) {
      values = line.substring(3).trim().split(" ")
      material.emissiveColor = [parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]), 1.0]
    } else if (/^Kd /i.test(line)) {
      values = line.substring(3).trim().split(" ")
      material.diffuseColor = [parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]), 1.0]
    } else if (/^Ks /i.test(line)) {
      values = line.substring(3).trim().split(" ")
      material.specularColor = [parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]), 1.0]
...
function loadMaterialTexture(material, name, textureOptions, mtlDirectory, texturePromiseMap, texturePromises, options) {
  ...
  if (!defined(texturePromise)) {
    const shallowPath = join(mtlDirectory, basename(texturePath))
