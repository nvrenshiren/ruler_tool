...
describe("ktx2 transform browser", () => {
  ...
  test("filters textures by pattern", async () => {
    ...
    const texture1 = document.createTexture("color").setImage(new Uint8Array(imageBuffer)).setMimeType("image/png").setURI("color.png")

    const texture2 = document.createTexture("normal").setImage(new Uint8Array(imageBuffer)).setMimeType("image/png").setURI("normal.png")
