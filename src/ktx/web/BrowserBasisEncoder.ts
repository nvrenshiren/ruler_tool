...
class BrowserBasisEncoder {
    ...
    if (!promise) {
      function _init(): Promise<IBasisModule> {
        ...
        return new Promise((resolve, reject) => {
          Promise.all([import(/* @vite-ignore */ jsUrl), wasmUrl ? fetch(wasmUrl).then(res => res.arrayBuffer()) : undefined])
   ...
   */
  async encode(bufferOrBufferArray: Uint8Array | CubeBufferData, options: Partial<IEncodeOptions> = {}): Promise<Uint8Array> {
